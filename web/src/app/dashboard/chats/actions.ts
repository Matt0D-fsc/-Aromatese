...
async function updateConversation(conversationId: string, values: Record<string, unknown>) {
  const { supabase, tenant } = await staffSession();
...
export async function takeOver(conversationId: string) {
  await updateConversation(conversationId, { ai_muted: true, needs_human: false, taken_over_at: new Date().toISOString() });
}

export async function handBack(conversationId: string) {
  await updateConversation(conversationId, { ai_muted: false, needs_human: false });
}

export async function dismissAlert(conversationId: string) {
  await updateConversation(conversationId, { needs_human: false });
}
...
export async function sendStaffReply(conversationId: string, formData: FormData) {
  ...
  await updateConversation(conversationId, { ai_muted: true, needs_human: false, last_staff_reply_at: now, last_message_at: now });
}
